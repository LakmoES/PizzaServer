...
namespace Pizza.Models
{
    public class ApiContext
    {
        ...
        public ApiContext()
        {
            ...
            Apis.Add(new API { requireToken = true, url = "Auth/GetNewToken", type = "POST (сейчас есть GET)", parameters = "-", description = "Получение нового access token", returns = "Json/false" });
            Apis.Add(new API { requireToken = true, url = "Auth/Logout", type = "POST (сейчас есть GET)", parameters = "int allSessions [Выйти из всех сессий (0/1)]", description = "Завершить сессию(-и). Параметр не является обязательным.", returns = "ok/wrong token" });
