...
namespace Pizza.Models
{
    public class ApiContext
    {
        ...
        public ApiContext()
        {
            ...
            Apis.Add(new API { requireToken = false, url = "Auth/Login", type = "POST", parameters = "string username [Имя пользователя], string password [Пароль.md5]", description = "Авторизация. Получение access token", returns = "Json/false" });
            ...
            Apis.Add(new API { requireToken = true, url = "User/Edit", type = "POST", parameters = "string password [Пароль], string email [Email], string name [Имя], string surname [Фамилия]", description = "Редактирование профиля пользователя.", returns = "ok/wrong token/nothing to edit" });
            ...
            Apis.Add(new API { requireToken = true, url = "ShopCart/ProductExists", type = "POST", parameters = "int productID [ID товара]", description = "Проверка наличия товара в корзине текущего пользователя.", returns = "true/false/wrong token/bad argument" });
